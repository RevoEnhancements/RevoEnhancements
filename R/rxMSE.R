#
# RevoEnhancements/R/rxMSE by Chibisi Chima-Okereke
#
...
#' \code{rxMSE} Calculates mean squared error from predicted and response variable.
#'
...
#' rxMSE("Sepal.Length", "Sepal.Length_Pred", dat)
#' 
...
#' rxMSE("Sepal.Length", "Sepal.Length_Pred", dataFile)
#' file.remove(dataFile) 

rxMSE <- function (actualVarName, predVarName, data, blocksPerRead = 1,
                   reportProgress = rxGetOption("reportProgress")) 
{
  ...
  ret <- rxDataStep(
    ...
    transformObjects = list(RSS = 0, N = 0)
  )
