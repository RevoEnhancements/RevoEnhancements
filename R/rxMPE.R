#
# RevoEnhancements/R/rxMPE by Chibisi Chima-Okereke
#
...
#' \code{rxMPE} Calculates mean percentage error from predicted and response variable.
#'
...
#' rxMPE("Sepal.Length", "Sepal.Length_Pred", dat)
#' 
...
#' rxMPE("Sepal.Length", "Sepal.Length_Pred", dataFile)
#' file.remove(dataFile) 

rxMPE <- function (actualVarName, predVarName, data, blocksPerRead = 1,
                   reportProgress = rxGetOption("reportProgress")) 
{
  ...
  N <- ret[["N"]]
  MPE <- 100*dSumPropError/N
  
  list(MPE = MPE, SumPropError = dSumPropError, N = N)
}
